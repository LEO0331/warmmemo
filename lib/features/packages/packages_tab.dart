            ...
            child: Column(
              ...
              children: [
                ...
                _PackageCard(
                  name: '城市極簡告別',
                  price: 'NT\$ 120,000',
                  target: '適合希望儀式簡單、在殯儀館或小型會館完成告別的家庭，重視流程順暢與清楚說明。',
                  items: const [
                    '24 小時遺體接運一次（市區內）',
                    '停柩冰存 3 日（公立殯儀館或合作會館）',
                ...
                _PackageCard(
                  name: '家庭溫馨告別',
                  price: 'NT\$ 220,000',
                  target: '適合希望有多一點停留與親友告別時間的家庭，重視場地氛圍與紀念感。',
                  items: const [
                    '上述「城市極簡告別」全部內容',
                    '會館廳別升級（可容納 50–80 人）',
                    '靈堂佈置升級（主題式花藝設計）',
                ...
                _PackageCard(
                  ...
                  target: '適合希望以環保、低碳方式完成身後安排的人，著重在理念與家人溝通。',
                  items: const [
                    '遺體接運與冰存（天數依合作場地規定）',
                    '簡約告別儀式一次（家祭 + 少量親友）',
                    '骨灰處理與樹葬／海葬流程諮詢安排',
                ...
                const _PackageCard(
                  name: 'QR 紀念碑 / 數位追思牌',
                  price: 'NT\$ 150,000',
                  target: '把現有墓碑、銅牌或家族追思空間延伸成可掃描的數位紀念入口，先完成紀念頁與 QR，再交由碑商製作。',
                  items: [
                    '建立公開紀念頁網址與專屬 QR Code',
                    '可下載 QR 圖交給碑商印製、蝕刻或雕刻',
                    '支援生平摘要、人生亮點與給後人的話',
                    '可同步顯示告別式時間、地點等訃聞摘要',
                    '提供高對比、尺寸與安裝位置建議',
                  ],
              ...
              child: FilledButton(
                onPressed: () {
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) =>
                          CheckoutPage(planName: name, priceLabel: price),
                    ),
                  );
                },
          ...
          return Column(
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ChoiceChip(
                    ...
                    onSelected: (_) =>
                        setState(() => _paymentStatusFilter = null),
                  ),
                  ...statuses.map(
                    (s) => ChoiceChip(
                      label: Text(s),
              ...
              FilterChip(
                ...
                onSelected: (value) =>
                    setState(() => _onlyPendingPayment = value),
              ),
              const SizedBox(height: 8),
                  ...
                  return StaggeredReveal(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text(
                        ...
                        '成交階段：${_conversionStep(order)}｜里程碑：${_latestMilestoneSummary(order)}\n'
                          ...
                          builder: (_) => AlertDialog(
                            ...
                            content: Column(
                              ...
                              children: [
                                ...
                                if (order.notes != null)
                                  SelectableText('備註：${order.notes}'),
                                if (order.proposal != null &&
                                    !order.proposal!.isEmpty) ...[
                                  const SizedBox(height: 8),
                                  const SelectableText('客戶提案：'),
                                  if ((order.proposal!.vendorPreference ?? '')
                                      .trim()
                                      .isNotEmpty)
                                    SelectableText(
                                      '供應商偏好：${order.proposal!.vendorPreference}',
                                    ),
                                  if ((order.proposal!.materialChoice ?? '')
                                      .trim()
                                      .isNotEmpty)
                                    SelectableText(
                                      '材質偏好：${order.proposal!.materialChoice}',
                                    ),
                                  if ((order.proposal!.schedulePreference ?? '')
                                      .trim()
                                      .isNotEmpty)
                                    SelectableText(
                                      '排程偏好：${order.proposal!.schedulePreference}',
                                    ),
                                ],
                                if (order.vendorAssignment != null &&
                                    !order.vendorAssignment!.isEmpty)
                                  SelectableText(
                                    '指派供應商：${order.vendorAssignment!.vendorName ?? '-'}',
                                  ),
                                if (order.materialSelection != null &&
                                    !order.materialSelection!.isEmpty)
                                  SelectableText(
                                    '最終材質：${order.materialSelection!.label ?? '-'} (${order.materialSelection!.tier ?? '-'})',
                                  ),
                                if (order.deliverySchedule.isNotEmpty)
                                  SelectableText(
                                    '交付里程碑：${_latestMilestoneSummary(order)}',
                                  ),
                              ...
                              TextButton.icon(
                                onPressed: () async {
                                  final buffer = StringBuffer()
                                    ...
                                    ..writeln('成交階段：${_conversionStep(order)}')
                                    ..writeln(
                                      '交付里程碑：${_latestMilestoneSummary(order)}',
                                    );
                                  if (order.paymentProvider != null) {
                                  ...
                                  if (order.verificationLogs.isNotEmpty) {
                                    ...
                                    for (final log
                                        in order.verificationLogs.reversed) {
                                      buffer.writeln(
                                        '${log.actedAt.toLocal().toString().split('.').first}｜${log.actor}｜${log.summary}',
                                      );
                                    }
...
class _NotificationCenterCard extends StatefulWidget {
  ...
  @override
  State<_NotificationCenterCard> createState() =>
    ...
    return SectionCard(
      ...
      child: StreamBuilder<List<NotificationEvent>>(
        ...
        builder: (context, snapshot) {
          ...
          final filtered = _onlyUnread
              ? events.where((e) => e.status != 'read').toList()
              : events;
          final preview = filtered.take(8).toList();
          if (events.isEmpty) {
